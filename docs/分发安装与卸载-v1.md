...
1. `--version <tag>`：安装必填。
2. `--asset-base <url>`：可选，发布资产基地址（例如 `https://<domain>/releases`）。
3. `--acme-email <email>`：安装必填；可由 `YC_ACME_EMAIL` 兜底。
4. `--public-ip <ipv4>`：可选，优先级最高。
5. `--acme-staging`：可选，启用 LE staging。
6. `--dry-run`：可选。
7. `--yes`：可选，跳过确认。
8. `--purge`：可选，仅用于 `uninstall`。
9. `--format <text|json>`：可选，仅用于 `doctor`。

### 4.2 `sidecar.sh`

1. `--version <tag>`：安装必填。
2. `--asset-base <url>`：可选，发布资产基地址（例如 `https://<domain>/releases`）。
3. `--relay <wss-url>`：安装时建议必填；若缺失则尝试复用已持久化 relay。
4. `--allow-insecure-ws`：可选，仅调试用途。
5. `--dry-run`：可选。
6. `--yes`：可选。
...
1. relay 固定监听 `127.0.0.1:18080`。
2. sidecar 健康检查默认监听 `0.0.0.0:18081`（可通过 `SIDECAR_ADDR` 覆盖）。
3. nginx 对外监听 `80/443`，并反代：
   - `/healthz` -> relay
   - `/v1/ws` -> relay
   - `/` -> relay
4. `80` 端口仅允许 `/.well-known/acme-challenge/*`，其他路径统一 `404`。
5. ACME 固定为 `HTTP-01 + webroot`，不使用 DNS-01。
6. lego 固定参数：
   - `--accept-tos`
   - `--email <acme-email>`
   - `--domains <public-ip>`
...
2. 触发方式：
   ...
   - 或手工触发并指定 tag
3. 同步路径规则：
   - `oss://<bucket>/<prefix>/<tag>/relay-sidecar.sh`
   - `oss://<bucket>/<prefix>/<tag>/sidecar.sh`
...
5. 国内用户下载地址模板：
   - `https://<ALIYUN_OSS_DOMAIN>/<ALIYUN_OSS_PREFIX>/<tag>/relay-sidecar.sh`
   - `https://<ALIYUN_OSS_DOMAIN>/<ALIYUN_OSS_PREFIX>/<tag>/sidecar.sh`
...
## 14. 跨宿主联调测试（A 云主机 + B 本地 Mac + iOS 模拟器）

### 14.1 前置条件

1. A 机：Linux + 公网 IPv4，可访问 `80/443`。
2. B 机：本地 Mac mini（开发环境），可运行 `yc-sidecar` 或 `cargo run -p yc-sidecar`。
3. iOS：使用 B 机的 Simulator。

### 14.2 A 机安装（网关+执行机脚本）

```bash
TAG=vX.Y.Z
BASE=https://<ALIYUN_OSS_DOMAIN>/<ALIYUN_OSS_PREFIX>

curl -fsSL "${BASE}/${TAG}/relay-sidecar.sh" -o relay-sidecar.sh
...
### 14.3 B 机连接 A 的 Relay（本地 sidecar）
...
1. 在 Mobile 中点击“添加宿主机”。
2. 粘贴配对链接（来自 A 或 B 侧 `yc-sidecar pairing show --format link --relay <wss>` 输出）。
3. 配对成功后进入运维页，检查 Host Snapshot 和 Connected Tools。
