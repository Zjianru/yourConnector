...
### M2.1 代码模块化收敛（`IN_PROGRESS`）

- relay 从单文件拆分为 `api/auth/pairing/ws/state/app` 模块目录
- sidecar 从单文件拆分为 `session/pairing/tooling/discoverers` 模块目录
- mobile UI 由 `index.html` 内联脚本样式改为外部 `styles/*` 与 `js/*`（ES Modules）
