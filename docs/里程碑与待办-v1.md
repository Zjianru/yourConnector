...
- 代码门禁通过：`cargo fmt --check`、`cargo clippy -D warnings`、`cargo test --workspace`、`JS ESM 语法检查`
...
### M4.2 工具接入核心组件（`ACCEPTED`）

- sidecar 建立 Tool Adapter Core（统一发现、详情采集、缓存、去抖、stale）
- OpenClaw 与 OpenCode 统一接入 `tooling/adapters/*`
- 新增 `tool_details_snapshot` / `tool_details_refresh_request` 事件通道
- OpenClaw 改为专属摘要/详情分层（渠道身份 + 会话诊断 + 混合用量头条，详情页为五屏 + Sessions 三分段）
- OpenCode 继续独立详情模板，避免与 OpenClaw 内容混用
- 详情采集失败时保留旧值并标记“数据过期”，状态点同步为 stale
- OpenClaw v3.2 补丁：Sessions 三分段高信息密度化 + 详情页顶部空白收敛（紧凑布局）

### M4.3 系统日志与归档基线（`ACCEPTED`）

- relay 与 sidecar 默认写入 `logs/raw/<service>.log.YYYY-MM-DD`
- 启动时 + 周期任务自动归档历史日志到 `logs/archive/YYYY-MM-DD.7z`
- 同时保留 stdout 输出，支持开发联调
- 文件日志级别与 `RUST_LOG` 解耦（`YC_FILE_LOG_LEVEL` 默认 `debug`）
- 引入统一 `traceId`，mobile/relay/sidecar 可按同一链路追踪操作闭环
- mobile 调试页支持结构化操作日志复制（JSON）
...
- 支持工具级运维命令（如重启、状态采集）
- 输出结构化执行结果与错误码
- OpenClaw 文件修改“记忆与重放”能力（保留操作轨迹、支持手动重放/回滚）后置到本里程碑实现
