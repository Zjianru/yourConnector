# yourConnector 质量门禁与检查规范

## 1. 目标

1. 将“编译、静态检查、测试、文档一致性”固化为阻断型门禁。
2. 保证重构与迭代不会破坏既有行为基线。
...
5. `find app/mobile/ui/js -name '*.js' -print0 | xargs -0 -I{} sh -c 'node --check "$$1" && node --check --input-type=module < "$$1"' _ "{}"`
