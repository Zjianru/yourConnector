...
1. A34 启动 relay 与 sidecar 后，`logs/raw` 出现当日日志文件（`relay.log.YYYY-MM-DD`、`sidecar.log.YYYY-MM-DD`）。
...
1. A45 `yc-relay.sh` 与 `yc-sidecar.sh` 均仅支持 `install/uninstall/status/doctor/start/stop/restart`，脚本层不存在 `run`。
2. A46 `yc-relay.sh install` 未传 `--public-ip` 时可交互输入；`--yes` 模式下缺失公网 IP 必须失败（code=2）。
3. A47 `yc-relay.sh` 拒绝私网/回环/CGNAT 等非法 IPv4。
4. A48 `yc-sidecar.sh install --relay-ip` 自动拼接 `wss://<ip>/v1/ws` 并写入 sidecar 配置。
5. A49 `yc-relay run` 与 `yc-sidecar run` 仅属于二进制命令，不属于安装脚本命令。
6. A50 `yc-relay status/doctor/service/version` 命令可用，且 `status` 在服务非 active 时返回非 0。
7. A51 `yc-sidecar status/doctor/service/version` 命令可用，且保留 `relay/pairing` 子命令。
8. A52 Linux relay 安装后 `curl http://<ip>/.well-known/acme-challenge/ping` 可达，且 `curl http://<ip>/` 返回 `404`。
9. A53 Linux relay 证书 SAN 包含公网 IP；注入坏证书并 renew 后 `active` 必须回滚。
10. A54 `uninstall` 默认彻底清理；`--keep-data` 保留配置与身份。
11. A55 GitHub Release 页面仅展示 `yc-relay.sh`、`yc-sidecar.sh`、`release-manifest.json`、`release-checksums.txt`。
12. A56 OSS 路径包含 Linux/macOS 双架构 relay/sidecar 二进制与 `checksums.txt`。
