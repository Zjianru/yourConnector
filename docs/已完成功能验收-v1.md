...
### 4.9 Linux 分发脚本与证书链路（v3.3）

1. A45 `relay-sidecar.sh install` 未传 `--acme-email` 时必须失败并返回错误（code=2）。
2. A46 `relay-sidecar.sh` 公网探测优先级满足：`--public-ip` > `state.json` > 自动探测。
3. A47 `relay-sidecar.sh` 拒绝私网/回环/CGNAT 等非法 IPv4。
4. A48 `curl http://<ip>/.well-known/acme-challenge/ping` 可达。
5. A49 `curl http://<ip>/` 返回 `404`（非 challenge 路径）。
6. A50 `openssl x509 -text` 可验证 SAN 包含 `<ip>`。
7. A51 nginx 已存在其他 `default_server` 时，安装失败并打印冲突文件路径。
8. A52 注入坏证书并触发 renew 后，`active` 链接回滚，WSS 链路不中断。
9. A53 `relay-sidecar.sh doctor --format json` 输出包含 `ports/certs/ip/state/nginx/systemd/nextRetryAt`。
10. A54 `sidecar.sh install` 在未传 `--relay` 且已存在持久化配置时可复用旧 relay。
11. A55 `yc-sidecar relay set/-change/test/reset` 与 `yc-sidecar pairing show --format text|json|link|qr` 全部可用。
12. A56 `uninstall` 保留身份与证书，`uninstall --purge` 清理身份、状态、证书和日志。
