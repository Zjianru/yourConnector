...
1. 核心调度：`/Users/codez/develop/yourConnector/services/sidecar/src/tooling/core/mod.rs`
2. 缓存与 stale：`/Users/codez/develop/yourConnector/services/sidecar/src/tooling/core/cache.rs`
3. 调度参数：`/Users/codez/develop/yourConnector/services/sidecar/src/tooling/core/scheduler.rs`
4. 适配器注册：`/Users/codez/develop/yourConnector/services/sidecar/src/tooling/adapters/mod.rs`
5. OpenClaw 适配器：`/Users/codez/develop/yourConnector/services/sidecar/src/tooling/adapters/openclaw.rs`
6. OpenCode 适配器：`/Users/codez/develop/yourConnector/services/sidecar/src/tooling/adapters/opencode.rs`
...
7. `health --json` 固定纳入慢层采集（不再仅限深采），用于渠道用户名与健康摘要。

## 5. 详情事件协议

1. sidecar -> app：`tool_details_snapshot`
2. app -> sidecar：`tool_details_refresh_request`
3. 协议类型定义：`/Users/codez/develop/yourConnector/protocol/rust/src/lib.rs`
4. 公共 Envelope 字段：
   - `toolId`
...
4. `usage`
   - `providerWindows[]`（账号窗口真实值）
   - `modelTotals[]`（本地会话聚合）
   - `estimatedCost[]`（配置费率估算）
   - `balances[]`（可选余额快照）
   - `coverage`（覆盖说明）
5. `systemService`
   - `memoryIndex[]`
...
2. Sidecar 详情链路关键文件：
   - `/Users/codez/develop/yourConnector/services/sidecar/src/session/loop/mod.rs`
   - `/Users/codez/develop/yourConnector/services/sidecar/src/session/snapshots.rs`
   - `/Users/codez/develop/yourConnector/services/sidecar/src/control.rs`
3. 移动端详情渲染关键文件：
   - `/Users/codez/develop/yourConnector/app/mobile/ui/js/flows/connection-events.js`
   - `/Users/codez/develop/yourConnector/app/mobile/ui/js/views/tools.js`
   - `/Users/codez/develop/yourConnector/app/mobile/ui/js/modals/tool-detail.js`
