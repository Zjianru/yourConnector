...
1. `systemId`：宿主机标识。
2. `pairToken`：sidecar 长期配对令牌（仅配对入口使用）。
...
- 入参：`systemId`, `deviceId`, `pairToken?`, `pairTicket?`
- 出参：`ok`, `code`, `message`, `suggestion`, `data.authMode`

2. `POST /v1/pair/exchange`
- 入参：`systemId`, `deviceId`, `deviceName`, `pairToken?`, `pairTicket?`, `keyId`, `devicePubKey`, `proof`
...
1. 默认链路：`accessToken + keyId + ts + nonce + sig`。
2. 签名 payload：`ws\n{systemId}\n{deviceId}\n{keyId}\n{ts}\n{nonce}`。
3. 兼容链路：过渡期可接受 `pairTicket/pairToken`（仅兜底）。
