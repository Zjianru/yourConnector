...
- 入参：`systemId`, `pairToken`, `hostName?`, `relayWsUrl?`, `includeCode?`, `ttlSec?`
- 出参：`pairLink`, `pairTicket`, `relayWsUrl`, `systemId`, `hostName`, `pairCode?`, `simctlCommand`
- 用途：统一 sidecar banner 与 `scripts/pairing.sh` 的配对链接来源，避免本地脚本与 relay 生成不一致。
...
1. sidecar 不再本地生成 `pairTicket`，连接 relay 后调用 `/v1/pair/bootstrap` 获取展示信息。
2. `scripts/pairing.sh` 不再本地 `openssl` 生成票据，改为调用 relay 签发接口。
3. `pairTicket` 的签发来源统一为 relay，sidecar/banner/script 仅做展示与导入。

## 11. 工具详情扩展协议（Tool Adapter Core）

1. 新增 WS 事件：
- sidecar -> app：`tool_details_snapshot`
- app -> sidecar：`tool_details_refresh_request`
2. 事件类型结构定义位于：
`/Users/codez/develop/yourConnector/protocol/rust/src/lib.rs`
3. `tool_details_snapshot` 的 `details[]` 为统一 Envelope：
- `toolId`
- `schema`
- `stale`
- `collectedAt`
...
- `profileKey`
- `data`
4. `tool_details_refresh_request` 支持：
- `toolId`（可选，空表示刷新当前宿主机全部已接入工具）
- `force`（可选，是否绕过去抖强制刷新）
5. 当前支持 schema：
- `openclaw.v1`：agent/session/usage/gateway/memory 运行态
- `opencode.v1`：session/model/token/usage 运行态
6. 安全边界：
- 详情仅采运行态，不采业务正文、不采 auth 明细。
- 采集失败时保留旧值并标记 `stale=true`，避免错误清空。
